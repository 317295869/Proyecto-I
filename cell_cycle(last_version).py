# NOTA: Este archivo se paso de Matlab a Python, pero no compila completamente 
# Archivo para la busqueda de atractores en el modelo de Li
##############################################################################################
# Proyecto: "Importancia del ciclo celular de la levadura y cómo los modelos
#            Booleanos pueden ayudar a identificar las fases de ese proceso
#            por medio de los atractores"
# Por: Eduardo Alexis Valencia Dorantes
# Referencias: https://github.com/kotiang54/bFGN_model
...
def unique_counts(msg):
    [a,b,c] = np.unique(msg,return_index=True,return_inverse=True,axis=0)
    idx = []
    for i in range(len(b)):
        idx = idx.append(np.count_nonzero(c == i))
...
for i in range(numStates):
    ...
    for p in range(iterations):
        ...
        while test == False & count < 50:
            ...
            count += 1
            print(count)
        #tmp_fixd_pts = tmp_fixd_pts.append(updt_states)
        tmp_fixd_pts.append(updt_states)

    
    #fixd_points = np.vstack((fixd_points, unique_counts(tmp_fixd_pts)))
    array2 = np.array(tmp_fixd_pts)
    fixd_point_temp=np.unique(array2,axis=0)
    fixd_points.append(fixd_point_temp)

fixd_points=np.array(fixd_points)   
#############################################################################################
############################ El codigo funciona hasta este punto ############################
#############################################################################################

[global_attractors, C, D] = np.unique(fixd_points, return_index=True,return_inverse=True,axis=0)
print(global_attractors)
basis_cnts = []
